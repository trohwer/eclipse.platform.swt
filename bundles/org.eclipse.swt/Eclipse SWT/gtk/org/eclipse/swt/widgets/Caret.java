...
boolean drawCaret () {
	...
	if (OS.USE_CAIRO) {
		...
		if (image != null && !image.isDisposed() && image.mask [imageRepSelection] == 0) {
			...
			Cairo.cairo_set_source_surface(cairo, image.surface [imageRepSelection], 0, 0);
	...
	if (image != null && !image.isDisposed() && image.mask [imageRepSelection] == 0) {
		...
		gdk_pixmap_get_size (image.pixmap [imageRepSelection], width, height);
	 	int nX = x;
		if ((parent.style & SWT.MIRRORED) != 0) nX = parent.getClientWidth () - width[0] - nX;
	 	OS.gdk_draw_drawable(window, gc, image.pixmap [imageRepSelection], 0, 0, nX, y, width[0], height[0]);
	} else {
		int nWidth = width, nHeight = height;
		if (nWidth <= 0) nWidth = DEFAULT_WIDTH;
		int nX = x;
